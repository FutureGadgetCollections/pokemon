/**
 * Sales Data Loader
 * Fetches sale data from Google Sheets and creates a filterable, sortable table
 */
...
(function() {
  ...
  const SALES_CSV = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vS7r5wMRpL4oYi6kauBMwelG9EKnM0EiMCZTYUuYxvZwBUuHW8T2qq9m3sXJhiSsdBpwG2-xulg1co5/pub?output=csv';
  ...
  function createTable(data) {
    if (!data || data.length === 0) {
      return '<div class="no-results">No sale records found.</div>';
    }
  ...
  window.exportToCSV = function() {
    ...
    a.href = url;
    a.download = `pokemon-sales-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  async function loadData() {
    ...
    try {
      const response = await fetch(SALES_CSV);
      ...
      container.innerHTML = '<div class="error-message">Error loading sale data. Please try again later.</div>';
    }
  }
